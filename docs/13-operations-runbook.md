...
프론트 배포는 Windows에서 `.\aws\deploy-front.ps1`, 백엔드는 `.\aws\deploy-back.ps1` (이미지 → tar → EC2 load → `up --force-recreate`). 이미 load된 이미지로만 기동할 때:
...
새 백엔드 이미지를 올릴 때는 Windows에서 `.\aws\deploy-back.ps1`을 실행합니다.
