...
5. 쓰기 작업(게시글 작성·댓글·업로드 세션)은 JWT가 필요하며 USER도 전부 사용할 수 있습니다. AI 답변 endpoint는 현재 410 `AI_REPLY_DISABLED`로 비활성화되어 있습니다. 게시글/댓글 **수정/삭제는 작성자 본인 또는 ADMIN만** 가능하고, 작성자 미지정(레거시) 글/댓글은 ADMIN만 수정/삭제할 수 있습니다.
...
- 공용 PostgreSQL 컨테이너: `yangyag-postgres` (전용 database `llm`, schema `llm`. 2026-08-27에 `auto` DB 스키마에서 이관, 컨테이너명은 `auto-postgres`에서 변경)
- `llm-front`, `llm-back`, `yangyag-postgres` 모두 Docker health 기준 `healthy`
