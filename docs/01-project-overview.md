...
3. 게시글 목록, 상세, 첨부파일 다운로드는 로그인 없이 접근할 수 있습니다.
4. 쓰기 작업, 댓글 작업, AI 답변 생성, 업로드 세션 API는 JWT가 필요합니다.
5. ZIP 업로드 도구는 ZIP 파일을 base64 청크로 나누고, alias 필드와 AES-GCM 암호문 JSON으로 백엔드에 전송합니다.
6. 서버는 청크를 임시 저장소에 보관하고 finalize 시 원본 ZIP을 재조립해 첨부파일 게시글을 생성합니다.
