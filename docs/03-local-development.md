...
로그인 성공 시 프론트는 `auth_token`, `auth_username`, `auth_user_id`, `auth_role`(`ADMIN`/`USER`), `auth_last_activity`를 `localStorage`에 저장합니다(`front/stores/auth.ts`). 인증 오류가 나면 다섯 값을 모두 삭제하고 로그인 화면으로 돌아갑니다. `auth_role`은 `/users` 화면 라우트 가드에, `auth_user_id`와 `auth_role`은 목록/상세의 수정·삭제 버튼 노출 판단(작성자 계정 ID 일치 또는 ADMIN)에 사용됩니다.
