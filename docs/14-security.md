...
현재 백엔드는 Spring Security filter chain을 쓰지 않는다. `auth` 외부 모듈의 보호 컨트롤러는 공개 계약인 `AuthenticationGateway.authenticate`를 호출하며, `auth.internal`의 `JwtProvider` 구현이 JWT와 현재 계정의 존재 여부를 검증한다. auth 내부 컨트롤러는 `JwtProvider`를 직접 사용할 수 있다.
