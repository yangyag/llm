...
- 요청 크기 한도는 설정값 외에 파싱 단계 한도가 따로 있다. 청크는 암호화 JSON 필드 하나라 Jackson 문자열 한도(2천만 자) 때문에 decode 후 11,249,976바이트가 최대이고, `APP_UPLOAD_SESSIONS_MAX_DECODED_CHUNK_SIZE`(기본 8MB)가 더 크면 기동 시 경고 후 이 값으로 낮춘다. multipart 텍스트 필드 합계는 `server.tomcat.max-http-form-post-size=8MB`(본문 codec 한도에 맞춘 값)이다. Tomcat 한도는 MockMvc로 재현되지 않으므로 실제 Tomcat 테스트(`HttpRequestLimitsTest`)로 확인한다. (docs/05, docs/08)
- 로그인은 클라이언트 IP마다 `APP_AUTH_LOGIN_WINDOW`(15m) 동안 `APP_AUTH_LOGIN_MAX_FAILURES`(10)번까지 시도할 수 있고 넘으면 429 `TOO_MANY_LOGIN_ATTEMPTS`(메모리 기록, 재시작 시 초기화). 클라이언트 IP는 `server.forward-headers-strategy=native`가 `X-Forwarded-For`에서 내부망 주소를 건너뛰어 복원한다. 앞단 프록시 구성(호스트 nginx → llm-front nginx)을 바꾸면 모든 요청이 같은 IP로 세어지지 않는지 확인한다. test `application.properties`는 main을 가리므로 이 설정도 옮겨 두었다. (docs/05, docs/14)
- front nginx: `/_nuxt/`는 없는 파일이면 404(SPA fallback 제외), `/api/`는 `proxy_read_timeout 600s`(큰 ZIP finalize). EC2 호스트 nginx(저장소 밖, 기본 60초)와 업로드 스크립트 `--timeout`(기본 30초)이 먼저 끊을 수 있다. (docs/08, docs/15)
- DB가 저장할 수 없는 값(SQLState 22xxx: NUL·길이 초과)은 409가 아니라 400 `INVALID_REQUEST`. 제목·본문·댓글·검색어의 NUL은 저장 전에 400으로 거부한다(H2 테스트 DB는 NUL을 받아주므로 운영 PostgreSQL과 다름).
...
- rich `body_document`(canonical 문서)에는 `blob:`/`data:`/외부 URL·`src` 원문을 저장하지 않고 `imageKey`(UUID) 참조만 남긴다. 서버 codec이 image node attribute를 `imageKey`·`alt`로 제한하고 unknown node/mark/attribute·raw HTML·`src`를 저장 전에 거부한다(`400 INVALID_RICH_DOCUMENT`). 프론트 serializer도 runtime 속성을 제거한다. 편집기 상태가 canonical 규칙을 벗어나면 v-model이 갱신되지 않아 이후 입력이 유실되므로, `front/components/post/postDocumentNormalizer.ts`가 입력 직후 바로잡고 그래도 변환이 안 되면 저장을 막는다. canonical 규칙을 바꿀 때는 이 정규화도 함께 맞춘다.
- 첨부 한도는 일반 첨부 + inline 이미지 **합계 게시글당 5개**(`APP_ATTACHMENTS_MAX_COUNT`), inline 이미지 파일당 **10MB**(`APP_ATTACHMENTS_INLINE_IMAGES_MAX_FILE_SIZE`)이고 PNG/JPEG만 허용한다(SVG/GIF/외부 URL 제외). 브라우저 pending registry는 편집 세션당 20개·100MB(`front/composables/useInlineImageDraft.ts`)로 초과 추가를 거부하고 저장 성공·취소(편집 패널)·unmount 시 `blob:` URL을 해제하고, 저장 실패 시에는 blob을 유지해 재시도한다. (docs/05, docs/07)
