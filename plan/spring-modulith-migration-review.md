...
**근거:** [AGENTS.md](../AGENTS.md) 24행은 새 보호 엔드포인트에서 `JwtProvider.authenticate`를 직접 호출하도록 안내한다. [docs/14-security.md](../docs/14-security.md) 5행, [docs/04-architecture.md](../docs/04-architecture.md) 100행과 계획서 150행에도 같은 설명이 남아 있다.

현재 `JwtProvider`는 `auth.internal`에 있고, 게시판·업로드 컨트롤러는 `auth.api.AuthenticationGateway`를 주입받는다. 지침대로 외부 모듈에서 `JwtProvider`를 직접 참조하면 이번에 도입한 내부 패키지 접근 금지와 충돌한다.

**정정할 내용:** “auth 외부 모듈의 보호 컨트롤러는 `AuthenticationGateway.authenticate`를 호출한다. auth 내부 구현인 `JwtProvider`가 JWT와 현재 계정 존재 여부를 검증한다.”로 명확히 한다. auth 내부 컨트롤러의 직접 사용은 그대로 설명할 수 있다. 인증 방식과 검증 동작을 바꾸는 작업은 필요하지 않다.

### 실패 상태 기록 시점

**근거:** 계획서 160행은 `catch`가 `UploadSessionFailureService.markFailed`를 `REQUIRES_NEW`로 호출한다고 설명한다. 실제 [UploadSessionService.java](../back/src/main/java/com/llm/app/upload/service/UploadSessionService.java) 186–188행은 콜백만 등록하며, 463–482행에서 `afterCompletion(STATUS_ROLLED_BACK)` 이후 별도 트랜잭션으로 실패를 기록한다.

**정정할 내용:** 다음 순서를 문서에 반영한다.
...
롤백 전 직접 호출과 롤백 후 호출은 PostgreSQL 행 잠금 측면에서 중요한 차이다. 코드의 현재 순서를 유지하고 문서를 정정해야 한다. 계획서 172행도 정리 콜백이 세션 삭제 후에 등록되는 것처럼 설명하지만, 현재는 조립 경로 생성 직후인 코드 158–159행에서 등록되므로 함께 수정한다.
